...
export const TiledRadioItem = (props: TiledRadioItemProps) => {
  ...
  return (
    <span className="flex flex-1 flex-col">
      <span className={`text-std-16N-7 text-body ${isDisabled ? "text-disabled" : ""}`}>{props.title}</span>
      {props.description && <span className={`text-caption-lg text-description ${isDisabled ? "text-disabled" : ""}`}>{props.description}</span>}
    </span>
  );
};
